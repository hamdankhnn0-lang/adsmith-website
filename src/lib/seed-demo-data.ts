...
import { BRANCH_SEED } from "@/lib/constants";
...
export async function seedDemoData(prisma: PrismaClient) {
  const log: string[] = [];
  const say = (msg: string) => { log.push(msg); console.log(msg); };

  say("Seeding Pizza Box Peshawar demo data...");
  ...
  await prisma.user.upsert({
    ...
    update: {},
    create: { name: "Super Admin", email: "admin@pizzabox.pk", passwordHash, role: "SUPER_ADMIN" },
  });

  await prisma.user.upsert({
    where: { email: "analyst@pizzabox.pk" },
    update: {},
    create: { name: "Reputation Analyst", email: "analyst@pizzabox.pk", passwordHash, role: "ANALYST" },
  });
  ...
  for (const b of BRANCH_SEED) {
    ...
    await prisma.googleLocation.upsert({
      ...
      update: {},
      create: { branchId: branch.id, locationName: `Pizza Box Peshawar - ${b.name}`, connected: false },
    });
  ...
  say("Generating demo Google reviews...");
  let reviewCounter = 0;
  ...
  for (const branch of branches) {
    ...
    for (let i = 0; i < reviewCount; i++) {
      ...
      if (roll < 0.55) { rating = 5; template = pick(POSITIVE_TEMPLATES); }
      else if (roll < 0.75) { rating = 4; template = pick(POSITIVE_TEMPLATES.concat(MIXED_TEMPLATES)); }
      else if (roll < 0.87) { rating = 3; template = pick(MIXED_TEMPLATES); }
      else if (roll < 0.95) { rating = 2; template = pick(NEGATIVE_TEMPLATES); }
      else { rating = 1; template = pick(NEGATIVE_TEMPLATES); }
    ...
    say(`  ${branch.name}: ${reviewCount} reviews generated`);
  }

  say("Generating demo private customer feedback...");
  for (const branch of branches) {
  ...
  say("Generating demo competitor data...");
